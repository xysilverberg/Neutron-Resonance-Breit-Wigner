...
% SLIDE 8 — TikZ Snake (Ular) Flowchart — Clean redesign
\begin{frame}{Pipeline Komputasi Numerik 7 Tahap}
% Styles defined inside frame to avoid global conflicts
\tikzset{
  fBox/.style={
    draw=UnairBlue, fill=UnairBlue!10, line width=1pt,
    rounded corners=5pt,
  ...
  fArrow/.style={
    ...
    draw=UnairYellow!80!orange, line width=2pt
  },
  fBend/.style={
    -{Stealth[length=6pt,width=5pt]},
    draw=DarkSlate!70, line width=2pt
  },
  fCap/.style={
    draw=DarkSlate, fill=DarkSlate!18, line width=1.2pt,
    ellipse,
...
% ═══════════════════════════════════════════════════════════════
% ROW 1  (y = 0)   :  MULAI → T1 → T2 → T3 → T4
% Node centers:  x = 0 / 3.6 / 7.2 / 10.8 / 14.4
% ═══════════════════════════════════════════════════════════════
\node[fCap]  (S)  at ( 0.0, 0) {MULAI\\[4pt]Data EXFOR};
\node[fBox]  (T1) at ( 3.6, 0) {%
  \textbf{\color{UnairBlue}Tahap 1}\\[5pt]%
  Akuisisi Data\\%
  IAEA-NDS EXFOR\\%
  \textit{Parser} ASCII\\%
  CSV ($N{=}59$)};
\node[fBox]  (T2) at ( 7.2, 0) {%
  \textbf{\color{UnairBlue}Tahap 2}\\[5pt]%
  Kalkulus Analitik\\%
  Jacobian $\mathbf{J}$\\%
  Gradien $\mathbf{g}$\\%
  Hessian $\mathbf{H}$};
\node[fBox]  (T3) at (10.8, 0) {%
  \textbf{\color{UnairBlue}Tahap 3}\\[5pt]%
  Tebakan Awal $\mathbf{p}_0$\\%
  $E_r^{(0)}{=}\arg\max\sigma$\\%
  $\Gamma^{(0)}$ via FWHM\\%
  $\sigma_{bg}^{(0)}{=}\min\sigma$};
\node[fBox]  (T4) at (14.4, 0) {%
  \textbf{\color{UnairBlue}Tahap 4}\\[5pt]%
  Newton-Raphson\\%
...
% ═══════════════════════════════════════════════════════════════
% ROW 2  (y = -4.0) :  T5 → T6 → T7 → SELESAI   (right→left)
% Node centers:  x = 14.4 / 10.8 / 7.2 / 3.6
% ═══════════════════════════════════════════════════════════════
\node[fBox]  (T5) at (14.4,-4.0) {%
  \textbf{\color{UnairBlue}Tahap 5}\\[5pt]%
  Statistik \&\\%
  Kovariansi\\%
  $\chi^2_{red}$,\; $\mathbf{C}{=}2\mathbf{H}^{-1}$\\%
  Korelasi $\boldsymbol{\rho}$};
\node[fBox]  (T6) at (10.8,-4.0) {%
  \textbf{\color{UnairBlue}Tahap 6}\\[5pt]%
  Noise Stress Test\\%
  Monte Carlo\\%
  $\eta{=}5\%,\,15\%,\,30\%$\\%
  Laju Sukses};
\node[fBox]  (T7) at ( 7.2,-4.0) {%
  \textbf{\color{UnairBlue}Tahap 7}\\[5pt]%
  Visualisasi\\%
  Kurva Fitting\\%
  Plot Konvergensi\\%
  Peta Derau};
\node[fCap]  (E)  at ( 3.6,-4.0) {SELESAI\\[4pt]Laporan \&\\Validasi};
...
% SNAKE TURN — T4 turun ke T5 (keduanya di x=14.4)
\draw[fBend]  (T4.south) -- (T5.north);
...
\begin{scope}[on background layer]
  \fill[UnairBlue!6, rounded corners=8pt]
    (-1.2, 1.55) rectangle (15.7, -5.55);
\end{scope}

% Direction label — row 1
\node[font=\fontsize{8pt}{9pt}\selectfont\bfseries\color{UnairBlue!55}]
  at (7.2, 1.28) {$\longrightarrow$ Baris 1: Tahap 1 -- 4};

% Direction label — row 2
\node[font=\fontsize{8pt}{9pt}\selectfont\bfseries\color{UnairBlue!55}]
  at (9.0, -5.28) {Baris 2: Tahap 5 -- 7 $\longleftarrow$};

% Turn indicator arrow
\node[font=\fontsize{10pt}{12pt}\selectfont\color{DarkSlate!70}]
  at (15.6, -2.0) {$\downarrow$};
